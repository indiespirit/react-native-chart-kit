  ...
  renderDots = config => {
    ...
    data.map((dataset, index) => {
      dataset.data.map((x, i) => {
        if (x !== null) {
          var values = datas.map(function (o) {
            return o;
          }).filter(function (val) {
            return val !== null
          });
          ...
          const cy =
            (height / 4) *
            3 *
            (1 - (x - Math.min(...values)) / this.calcScaler(datas)) +
            paddingTop
          const onPress = () => {
            if (!onDataPointClick) {
              return
            ...
            dataset.data
              .map(
                (x, i) => x !== null ?
                  ...
                  [(
                    paddingRight +
                    ((i !== 0 ? (i - 1) : (i)) * (width - paddingRight)) / dataset.data.length +
                    ',' +
                    ((height / 4) *
                      3 *
                      (1 - (x) / this.calcScaler(datas)) +
                      paddingTop)
                  ),
                  (
                    paddingRight +
                    ((i + 1) * (width - paddingRight)) / dataset.data.length +
                    ',' +
                    ((height / 4) *
                      3 *
                      (1 - (x) / this.calcScaler(datas)) +
          ...
          <G>
            ...
            <G>
              {this.renderHorizontalLabels({
                ...config,
                count: Math.min(...values) === Math.max(...values) ? 1 : 4,
                data: values,
                paddingTop,
                paddingRight
              })}
